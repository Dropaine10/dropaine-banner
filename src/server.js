...
.brand{display:flex;align-items:center;gap:12px}.brand img{width:54px;height:54px;object-fit:contain;background:transparent;border-radius:14px;padding:0}
...
app.get("/tmdb-image", auth, async (req, res) => {
  try {
    const raw = String(req.query.url || "");
    const u = new URL(raw);
    if (u.protocol !== "https:" || u.hostname !== "image.tmdb.org") {
      return res.status(400).send("Imagem inválida");
    }
    const resp = await axios.get(raw, {
      responseType: "arraybuffer",
      timeout: 12000,
      headers: { "User-Agent": "Dropaine-Banner/0.2" }
    });
...
app.get("/login", (req, res) => {
  ...
  <div style="max-width:430px;margin:8vh auto">
    <div class="card" style="text-align:center">
      <img src="/dropaine-logo.png" style="width:170px;max-width:65%;background:transparent;border-radius:22px;padding:0" alt="Dropaine">
...
app.get("/", auth, (req, res) => {
  res.send(layout("Painel", `
    <div class="card" style="margin-bottom:18px">
      <span class="badge">Versão 0.2</span>
...
app.get("/brand", auth, (req, res) => {
  ...
  res.send(layout("Minha Marca", `
    ...
    <div class="card">
      ...
      <div class="row">
        <div>
          <img src="${esc(b.logo_path)}?v=${Date.now()}" style="max-width:230px;max-height:230px;background:transparent;border-radius:18px;padding:0">
        </div>
  ...
  return async (req, res) => {
    ...
    const list = results.map(item => {
      ...
      return `<div class="result">
        <div>${poster ? `<img src="/tmdb-image?url=${encodeURIComponent(poster)}" loading="lazy">` : `<div style="width:100px;height:145px;background:#242436;border-radius:12px"></div>`}</div>
        <div>
...
async function makeBanner({title, subtitle, imageUrl, type, channel}) {
  ...
  let logoBuffer = null;
  try {
    logoBuffer = await sharp(logoFs).resize({ width: 220, height: 220, fit: "inside", withoutEnlargement: true }).png().toBuffer();
  } catch {}
...
const phone = phoneText
  ...
  <svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    ${titleSvg}
    <text x="72" y="1188" fill="#eeeeee" font-size="34" font-family="Arial, sans-serif">${xmlEsc(details)}</text>
    <rect x="62" y="1220" width="690" height="82" rx="20" fill="#000000" fill-opacity=".62"/>
    <text x="90" y="1274" fill="#ffffff" font-size="34" font-weight="700" font-family="Arial, sans-serif">${phone}</text>
  </svg>`);

  const comps = [{ input: svg, top: 0, left: 0 }];
  if (logoBuffer) comps.push({ input: logoBuffer, top: 42, left: 810 });
